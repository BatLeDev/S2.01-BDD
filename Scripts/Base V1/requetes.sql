...
-- 9) Quels sont les compteur qui n'ont aucun relevé journalier ?
SELECT numero
    FROM Compteur
        LEFT JOIN ReleveJournalier ON leCompteur = numero
    WHERE leCompteur IS NULL;
...
-- 18) Quels sont les compteurs ayant des relevés pour tous les week-end ?
/* Requete Oracle SQL
SELECT numero
    FROM Compteur
    WHERE NOT EXISTS (
